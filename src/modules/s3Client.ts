...
export class S3Manager {
  ...
  private initializeClient(): void {
    try {
      ...
      if (!this.endpoint || !this.region || !this.accessKeyId || !this.secretAccessKey || !this.bucketName) {
  ...
  public isConfigured(): boolean {
    return !!(this.endpoint && this.region && this.accessKeyId && this.secretAccessKey && this.bucketName);
  }
  ...
  private async sha256Hex(data: string | Uint8Array): Promise<string> {
    ...
    return Array.from(hashArray, byte => ('0' + byte.toString(16)).slice(-2)).join('');
  }

  private async signRequest(method: string, url: string, headers: Record<string, string>, body?: Uint8Array): Promise<Record<string, string>> {
    ...
    const contentHash = body ? await this.sha256Hex(body) : await this.sha256Hex(new Uint8Array(0));
  ...
  private async calculateSignature(date: string, stringToSign: string): Promise<string> {
    let key: Uint8Array = await this.hmacSha256(`AWS4${this.secretAccessKey}`, date);
    ...
    return Array.from(signatureBytes, byte => ('0' + byte.toString(16)).slice(-2)).join('');
  }
  ...
  private async hmacSha256Bytes(key: Uint8Array, data: string): Promise<Uint8Array> {
  ...
  public async uploadFile(
    ...
    try {
      ...
      const signedHeaders = await this.signRequest('PUT', url, headers, uint8Array);
      ...
      return new Promise((resolve, reject) => {
        xhr.onload = () => {
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            ztoolkit.log(`Failed to upload file ${key}: ${xhr.status} ${xhr.statusText}`);
            resolve(false);
          }
        };
      ...
      return new Promise<boolean>((resolve) => {
        ...
        xhr.onload = () => {
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            ztoolkit.log(`S3 connection test failed with status ${xhr.status}: ${xhr.statusText}`);
